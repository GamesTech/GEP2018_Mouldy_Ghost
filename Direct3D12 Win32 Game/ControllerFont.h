...
namespace DX
{
    ...
    inline void XM_CALLCONV DrawControllerString(_In_ DirectX::SpriteBatch* spriteBatch, _In_ DirectX::SpriteFont* textFont, _In_ DirectX::SpriteFont* butnFont,
                                                 _In_z_ wchar_t const* text, DirectX::XMFLOAT2 const& position, DirectX::FXMVECTOR color = DirectX::Colors::White, Vector2 scale = Vector2::One)
    {
        ...
        for (size_t ch = 0; ch < textLen; ++ch)
        {
            ...
            else
            {
                ...
                case '\n':
                    ...
                    outPos.x = position.x;
                    outPos.y += textFont->GetLineSpacing() * scale.x;
                    break;
